...
namespace RPG
{
    public class Enemy_Manager : Character
    {
        ...
        void turnFace()
        {
            if (CharacterAnimator.GetFloat("direction_x") < 0)
            {
                body.localScale = new Vector3(charScale, charScale, charScale);
            }
            else if (CharacterAnimator.GetFloat("direction_x") > 0)
            {
                body.localScale = new Vector3(-charScale, charScale, charScale);
            }
        }
        ...
        ****************************************************************************************************************/
        ...
        private void Death()
        {
            ...
            // create a blood explosion on death
            Instantiate(Resources.Load("deathBloodyEyes"), transform.position, Quaternion.identity);
        ...
        private void hit(Transform OtherCharacter, float joltAmount)
        {
            ...
            if (canBeJolted)
            {
                ...
                if (transform.GetChild(0).GetComponent<Animator>().GetFloat("direction_x") != 0)
                {
                    if (transform.GetChild(0).GetComponent<Animator>().GetFloat("direction_x") > 0)
                    {
                ...
                {
                    if (transform.GetChild(0).GetComponent<Animator>().GetFloat("direction_y") > 0)
                    {
        ...
        {
            var render = this.gameObject.transform.GetChild(0).GetComponent<SpriteRenderer>();
